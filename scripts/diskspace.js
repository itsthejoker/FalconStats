...
exec("mount | grep disk | awk \'{print $3}\'", (error, stdout, stderr)=> {
    let lines = stdout.split("\n").filter(l => l);
    let disks = [];
    lines.forEach((line, i) => {
        disks.push({"mountpoint": line, "nicemountpoint": line})
    });

    exec("df -h -P -g "+disks.map(d => d.mountpoint).join(" "), (error, stdout, stderr) => {
